...
function productmatrix() {
    ...
    function template() {
        return [
            ...
            '<div class="clearfix" ng-repeat="group in comboVariants" ng-show="specCount == 2 && (group | filter:{Show: true}).length > 0">',
            '<div class="row"><div class="col-xs-12"><img class="pull-left img-thumbnail img-responsive matrix-group-thumb" ng-src="{{group[0].LargeImageUrl}}"> <h3>{{group.DisplayName}}</h3></div></div>',
            '<div class="row row-gutter matrix-grid">',
